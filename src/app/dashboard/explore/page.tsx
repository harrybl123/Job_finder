...
export default function Home() {
  ...
  useEffect(() => {
    const checkReturningUser = async () => {
      ...
      console.log('🔄 Returning user detected:', user.id);
      setIsReturningUser(true);

      try {
        // Fetch saved recommendations
        const recResponse = await fetch(`/api/save-recommendation`);
        ...
        if (recResponse.ok) {
          ...
          if (data.recommendations && data.recommendations.length > 0) {
            ...
            // Restore user level
            if (latestRec.userLevel) {
              console.log('📊 Restoring user level:', latestRec.userLevel);
              setCurrentLevel(latestRec.userLevel);
            }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 relative overflow-hidden">
      ...
      {/* Welcome Back Banner - Always on top */}
      {isReturningUser && step === 'galaxy' && (
        <div className="fixed top-0 left-1/2 transform -translate-x-1/2 h-16 flex items-center z-[60] animate-fade-in pointer-events-none">
          <div className="bg-gradient-to-r from-blue-500/20 to-purple-500/20 backdrop-blur-lg border border-blue-400/30 rounded-2xl px-6 py-2 shadow-2xl pointer-events-auto">
            <p className="text-blue-100 text-sm font-medium flex items-center gap-2 whitespace-nowrap">
      ...
      {/* Main Content */}
      <div className="relative z-10 w-full max-w-[95%] mx-auto px-4 py-8 flex flex-col items-center min-h-screen pt-24">

        {isLoading ? (
          <div className="flex flex-col items-center justify-center min-h-[50vh]">
            <div className="w-12 h-12 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin mb-4"></div>
            <p className="text-blue-200 animate-pulse">Loading your career galaxy...</p>
          </div>
        ) : (
